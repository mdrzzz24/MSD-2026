...
<head>
    ...
    <title>Attendee Pass — {{ $registrant->name }}</title>
...
<body class="bg-[#050d2a] min-h-screen flex items-center justify-center p-4 sm:p-8">
    ...
    <div class="w-full max-w-3xl">
        ...
        <div class="relative overflow-hidden rounded-t-3xl shadow-2xl bg-[#0e2461]">
            <img src="{{ asset('img/QRHeader.png') }}" alt="Metrodata Solution Day 2026" class="w-full h-auto block">
        ...
        <div class="bg-white rounded-b-3xl shadow-2xl overflow-hidden">
            <div class="grid sm:grid-cols-2">
                ...
                <div class="p-6 sm:p-8 flex flex-col items-center justify-center text-center border-b sm:border-b-0 sm:border-r border-gray-200">
                    ...
                    <h2 class="text-xl sm:text-2xl font-extrabold text-gray-900 mt-2">{{ $registrant->name }}</h2>
                    <p class="text-sm font-medium text-gray-500 mt-0.5">{{ $registrant->company ?: $registrant->job_title }}</p>
                    <div class="mt-5 bg-white p-3 rounded-2xl border-2 border-dashed border-[#ff3d6e]/40">
                        <img src="{{ str_replace('size=150x150', 'size=400x400', $registrant->qr_code_url) }}" alt="QR Code" class="w-44 h-44 mx-auto">
                    </div>
                    <p class="mt-3 text-xs text-gray-400 font-mono">{{ $registrant->unique_code }}</p>
