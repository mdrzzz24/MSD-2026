...
<head>
    ...
    <title>Unbalanced Data — {{ config('app.name') }}</title>
    ...
    <main class="flex-1 lg:ml-64">
        ...
        <div class="p-4 sm:p-6 lg:p-8 space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <a href="{{ route('admin.regist-confirmation.unbalanced', ['reason' => 'all']) }}"
                   class="group bg-white rounded-2xl p-5 border border-red-100 shadow-sm hover:shadow-md transition-shadow {{ $reasonFilter === 'all' ? 'ring-2 ring-red-500' : '' }}">
                ...
                <a href="{{ route('admin.regist-confirmation.unbalanced', ['reason' => 'status']) }}"
                   class="group bg-white rounded-2xl p-5 border border-amber-100 shadow-sm hover:shadow-md transition-shadow {{ $reasonFilter === 'status' ? 'ring-2 ring-amber-500' : '' }}">
                    <div class="flex items-center justify-between mb-3">
                    ...
                    <p class="text-3xl font-bold text-amber-700">{{ $stats['status'] }}</p>
                    <p class="text-xs text-amber-500 mt-1">Status vs recommendation</p>
                </a>
                <a href="{{ route('admin.regist-confirmation.unbalanced', ['reason' => 'waitlist_flag']) }}"
                   class="group bg-white rounded-2xl p-5 border border-purple-100 shadow-sm hover:shadow-md transition-shadow {{ $reasonFilter === 'waitlist_flag' ? 'ring-2 ring-purple-500' : '' }}">
                    <div class="flex items-center justify-between mb-3">
                    ...
                    <p class="text-3xl font-bold text-purple-700">{{ $stats['waitlist_flag'] }}</p>
                ...
                <div class="px-5 py-4 border-b border-gray-100">
                    <div class="flex flex-col sm:flex-row sm:items-center gap-3">
                        ...
                        <div class="flex flex-wrap items-center gap-2 sm:ml-auto">
                            <form method="GET" action="{{ route('admin.regist-confirmation.unbalanced') }}" class="flex items-center gap-2 mr-1">
                                ...
                                <div class="relative">
                                    ...
                                    @if ($search !== '')
                                        <a href="{{ route('admin.regist-confirmation.unbalanced', array_merge(request()->except(['search', 'page']), ['reason' => $reasonFilter])) }}"
                            ...
                            <div class="flex items-center gap-1.5 text-xs">
                                ...
                                <a href="{{ route('admin.regist-confirmation.unbalanced', array_merge(request()->except(['reason', 'page']), ['reason' => 'all'])) }}" class="px-2.5 py-1 rounded-lg font-semibold transition {{ $reasonFilter === 'all' ? 'bg-gray-800 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200' }}">All</a>
                                <a href="{{ route('admin.regist-confirmation.unbalanced', array_merge(request()->except(['reason', 'page']), ['reason' => 'status'])) }}" class="px-2.5 py-1 rounded-lg font-semibold transition {{ $reasonFilter === 'status' ? 'bg-amber-500 text-white' : 'bg-amber-50 text-amber-700 hover:bg-amber-100' }}">Status Conflict</a>
                                <a href="{{ route('admin.regist-confirmation.unbalanced', array_merge(request()->except(['reason', 'page']), ['reason' => 'waitlist_flag'])) }}" class="px-2.5 py-1 rounded-lg font-semibold transition {{ $reasonFilter === 'waitlist_flag' ? 'bg-purple-500 text-white' : 'bg-purple-50 text-purple-700 hover:bg-purple-100' }}">Waitlist Flag</a>
                            </div>
                        </div>
                    </div>
                </div>
                                ...
                                <tr class="hover:bg-gray-50/50 transition">
                                    ...
                                    <td class="px-5 py-3">
                                        <div class="flex flex-wrap gap-1">
                                            @php $reasons = $r->unbalancedReasons(); @endphp
                                            @forelse ($reasons as $key)
                                                <span class="px-2 py-0.5 rounded-full text-[10px] font-semibold {{ $reasonLabels[$key][1] ?? 'bg-gray-100 text-gray-600' }}">{{ $reasonLabels[$key][0] ?? $key }}</span>
                                            @empty
