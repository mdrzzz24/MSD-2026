...
<head>
    ...
    <title>Admin Emails — {{ config('app.name') }}</title>
        ...
        <header class="sticky top-0 z-30 bg-white/80 backdrop-blur border-b border-gray-200">
            <div class="flex items-center justify-between h-16 px-4 sm:px-6 lg:px-8">
                ...
                <div class="flex items-center gap-2">
                    <a href="{{ route('admin.admin-emails.create') }}"
                                ...
                                <tr class="hover:bg-gray-50/50 transition">
                                    <td class="px-5 py-4">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <span class="text-sm font-semibold text-gray-900">{{ $ae->name }}</span>
                                        </div>
                                    </td>
                                    <td class="px-5 py-4">
                                        <span class="text-sm text-gray-600 font-mono">{{ $ae->email }}</span>
                                    </td>
                                    <td class="px-5 py-4">
                                        @if ($ae->is_active)
                                    ...
                                    <td class="px-5 py-4">
                                        <div class="flex justify-center gap-1.5">
                                            <a href="{{ route('admin.admin-emails.edit', $ae) }}"
                                               class="px-2.5 py-1.5 text-xs font-medium rounded-lg bg-amber-100 text-amber-700 hover:bg-amber-200 transition">Edit</a>
                                            <form action="{{ route('admin.admin-emails.destroy', $ae) }}" method="POST" onsubmit="return confirm('Delete admin email {{ $ae->email }}?')">
                                                @csrf @method('DELETE')
