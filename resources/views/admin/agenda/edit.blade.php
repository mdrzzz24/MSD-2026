    ...
    <form action="{{ route('admin.agenda.update', $agendum) }}" method="POST" class="space-y-4">
        ...
        <div class="border-t border-gray-100 pt-4 mt-2">
            ...
            <div class="mt-3">
                <label class="flex items-center gap-2 cursor-pointer">
                    ...
                    <input type="checkbox" name="registration_closed" value="1" {{ old('registration_closed', $agendum->registration_closed) ? 'checked' : '' }} class="w-4 h-4 rounded border-gray-300 text-red-600 focus:ring-red-500">
        ...
        <div class="border-t border-gray-100 pt-4 mt-2">
            ...
            @php
                ...
                // Speakers in display order for the reorder list. Uses the submitted
                // order (on validation errors) or the stored pivot order; if no order
                // was ever stored (all 0), fall back to alphabetical by default.
                $byId = $allSpeakers->keyBy('id');
                $orderedSpeakerModels = collect($selectedSpeakers)->map(fn($id) => $byId[$id] ?? null)->filter()->values();
                ...
                <div class="space-y-3 max-h-80 overflow-y-auto border border-gray-200 rounded-xl p-3 bg-gray-50">
                    ...
                    <div class="flex items-start gap-3 p-2 rounded-lg hover:bg-white transition">
                        <label class="flex items-center gap-2 cursor-pointer flex-shrink-0 pt-1">
                            <input type="checkbox" value="{{ $sp->id }}" data-name="{{ $sp->name }}" {{ in_array($sp->id, $selectedSpeakers) ? 'checked' : '' }} onchange="toggleSpeaker(this)" class="w-4 h-4 rounded border-gray-300 text-indigo-600 speaker-check">
                        </label>
