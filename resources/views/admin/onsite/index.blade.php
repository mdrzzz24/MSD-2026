...
<span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold border {{ $mqttEnabled ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-gray-100 text-gray-500 border-gray-200' }}" title="Topik MQTT badge printer">
    <span class="w-2 h-2 rounded-full {{ $mqttEnabled ? 'bg-emerald-500 animate-pulse' : 'bg-gray-400' }}"></span>
    <span class="font-mono">{{ $mqttTopic }}</span>
    <span class="ml-0.5 {{ $mqttEnabled ? 'text-emerald-600' : 'text-gray-400' }}">{{ $mqttEnabled ? 'ON' : 'OFF' }}</span>
</span>
