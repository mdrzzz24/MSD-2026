...
<div class="p-4 sm:p-6 lg:p-8"><div class="max-w-2xl"><div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 sm:p-8">
    ...
    @endif
    @if (session('error'))
        <div class="flex items-start gap-3 bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-xl mb-6"><svg class="w-5 h-5 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg><span class="text-sm">{!! session('error') !!}</span></div>
    @endif
    @if ($errors->any())
    ...
    <form action="{{ route('admin.templates.upload.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
        ...
        <div><label class="block text-sm font-semibold text-gray-700 mb-1.5">File</label><p class="text-xs text-gray-400 mb-2">Pilih file <strong>.html</strong> / <strong>.htm</strong> langsung, atau <strong>.zip</strong> dari export Word (File → Save as → Web Page).</p><p class="text-xs text-gray-400 mb-2">Placeholder yang didukung: <code class="bg-gray-100 px-1 py-0.5 rounded text-indigo-600">@{{ name }}</code> <code class="bg-gray-100 px-1 py-0.5 rounded text-indigo-600">@{{ email }}</code> <code class="bg-gray-100 px-1 py-0.5 rounded text-indigo-600">@{{ status }}</code> <code class="bg-gray-100 px-1 py-0.5 rounded text-indigo-600">@{{ password }}</code> <code class="bg-gray-100 px-1 py-0.5 rounded text-indigo-600">@{{ admin_notes }}</code></p><input type="file" name="html_file" accept=".html,.htm,.zip" required class="w-full text-sm file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100 transition"></div>
