...
<div class="p-4 sm:p-6 lg:p-8"><div class="max-w-2xl"><div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 sm:p-8">
    ...
    <form action="{{ route('admin.templates.upload.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
        ...
        <div><label class="block text-sm font-semibold text-gray-700 mb-1.5">Tipe</label><select name="type" required class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 focus:bg-white transition"><option value="">-- Select --</option><option value="approval" {{ old('type')==='approval'?'selected':'' }}>Approval</option><option value="rejection" {{ old('type')==='rejection'?'selected':'' }}>Rejection</option></select></div>
