...
<head>
    ...
    <title>Email Templates — {{ config('app.name') }}</title>
        ...
        <header class="sticky top-0 z-30 bg-white/80 backdrop-blur border-b border-gray-200">
            <div class="flex items-center justify-between h-16 px-4 sm:px-6 lg:px-8">
                ...
                <div class="flex items-center gap-2">
                    @php $autoEmail = \Illuminate\Support\Facades\Cache::get('auto_registration_email', true); @endphp
                    <form action="{{ route('admin.templates.toggle-auto-email') }}" method="POST">
                        @csrf
                        <button type="submit" class="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-medium rounded-xl transition-all {{ $autoEmail ? 'bg-emerald-50 text-emerald-700 border border-emerald-200 hover:bg-emerald-100' : 'bg-gray-100 text-gray-500 border border-gray-200 hover:bg-gray-200' }}">
                            <span class="w-1.5 h-1.5 rounded-full {{ $autoEmail ? 'bg-emerald-500 animate-pulse' : 'bg-gray-400' }}"></span>
                            Auto-Email: {{ $autoEmail ? 'ON' : 'OFF' }}
                        </button>
                    </form>

                    <a href="{{ route('admin.templates.upload') }}"
        ...
        <div class="p-4 sm:p-6 lg:p-8">
                ...
                <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xl font-bold text-gray-900">{{ $totalTemplates }}</p>
                        </div>
                    </div>
                </div>
                <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xl font-bold text-emerald-600">{{ $activeCount }}</p>
                        </div>
                    </div>
                </div>
                <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xl font-bold text-gray-900">{{ $typeCount }}</p>
                        </div>
                    </div>
                </div>
                <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xl font-bold text-amber-600">{{ $typeCount - count($activeTypes) }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-2 mb-4 px-1">
                ...
                @foreach ($types as $key => $info)
                    @php $hasActive = in_array($key, $activeTypes); @endphp
                    <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium {{ $hasActive ? 'bg-' . $info['color'] . '-50 text-' . $info['color'] . '-700 border border-' . $info['color'] . '-200' : 'bg-gray-50 text-gray-400 border border-gray-200' }}">
                        <span class="w-1.5 h-1.5 rounded-full {{ $hasActive ? 'bg-' . $info['color'] . '-500' : 'bg-gray-300' }}"></span>
                        {{ $info['label'] }}
                        @if ($hasActive)
                            <svg class="w-3 h-3 text-{{ $info['color'] }}-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/></svg>
                        @endif
                    </span>
                @endforeach
            </div>
                        ...
                        <tbody class="divide-y divide-gray-50">
                            @forelse ($templates as $t)
                                ...
                                <tr class="hover:bg-gray-50/50 transition table-row-transition group">
                                    <td class="px-5 py-4">
                                        <div class="flex items-center gap-3">
                                            <div class="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 bg-{{ $color }}-50 text-{{ $color }}-600 group-hover:scale-105 transition">
                                    ...
                                    <td class="px-5 py-4">
                                        <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-{{ $color }}-50 text-{{ $color }}-700 border border-{{ $color }}-200">
                                            <span class="w-1.5 h-1.5 rounded-full bg-{{ $color }}-500"></span>
                                            {{ $label }}
                                        </span>
                                    </td>
                                    <td class="px-5 py-4">
                                        <span class="text-sm text-gray-600 font-mono text-xs">{{ $t->subject }}</span>
                                    </td>
                                    <td class="px-5 py-4">
                                        @if ($t->is_active)
                                    ...
                                    <td class="px-5 py-4">
                                        <div class="flex items-center justify-end gap-1">
                                            {{-- Preview --}}
                                            <a href="{{ route('admin.templates.preview', $t) }}" target="_blank"
                                            ...
                                            {{-- Test --}}
                                            <button onclick="openTestModal({{ $t->id }}, '{{ $t->name }}')"
                                            ...
                                            <form action="{{ route('admin.templates.toggle', $t) }}" method="POST" class="inline">
                                                @csrf
                                                <button type="submit"
                                                   class="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium border transition-all {{ $t->is_active ? 'border-gray-200 text-gray-500 bg-white hover:border-amber-200 hover:text-amber-700 hover:bg-amber-50 hover:shadow-sm' : 'border-gray-200 text-gray-500 bg-white hover:border-emerald-200 hover:text-emerald-700 hover:bg-emerald-50 hover:shadow-sm' }}"
                                            ...
                                            {{-- Delete --}}
                                            <form action="{{ route('admin.templates.destroy', $t) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete &quot;{{ $t->name }}&quot;? This action cannot be undone.')">
                                                @csrf @method('DELETE')
                                                <button type="submit"
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mt-4">
                            @foreach ($types as $key => $info)
                                @php
                                    ...
                                    $color = $info['color'];
                                @endphp
                                <div class="rounded-xl border p-4 transition hover:shadow-sm {{ $hasActive ? 'bg-emerald-50/50 border-emerald-200' : 'bg-amber-50/50 border-amber-200' }}">
                                    <div class="flex items-center justify-between mb-2">
                                        <span class="text-xs font-semibold text-gray-600">{{ $info['label'] }}</span>
                                        <span class="w-2 h-2 rounded-full {{ $hasActive ? 'bg-emerald-500' : 'bg-amber-500' }}"></span>
                                    </div>
                                    @if ($hasActive)
                                    ...
                                    @else
                                        ...
                                        <a href="{{ route('admin.templates.create-from-fallback') }}?type={{ $key }}"
...
<div x-data="testModal()"
    ...
    <div class="flex items-center justify-center min-h-screen px-4">
        ...
        <div class="relative bg-white rounded-2xl shadow-xl border border-gray-100 w-full max-w-lg p-6">
            ...
            {{-- Modal Form --}}
            <form action="{{ route('admin.admin-emails.send-test') }}" method="POST">
                @csrf
                ...
                <div class="mb-5">
                    ...
                    @php $adminEmails = \App\Models\AdminEmail::active()->get(); @endphp
                    @forelse ($adminEmails as $ae)
                        <label class="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-gray-50 cursor-pointer transition border border-transparent hover:border-gray-100 mb-1">
                            <input type="checkbox" name="admin_email_ids[]" value="{{ $ae->id }}"
                                   class="w-4 h-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                            <div>
                                <span class="text-sm font-medium text-gray-800">{{ $ae->name }}</span>
                                <span class="text-xs text-gray-400 ml-1.5">({{ $ae->email }})</span>
                            </div>
                        </label>
                    @empty
                        <div class="text-center py-8">
                            ...
                            <a href="{{ route('admin.admin-emails.create') }}" class="text-sm text-indigo-600 hover:text-indigo-800 font-medium">Add recipient →</a>
                        </div>
                    @endforelse
                </div>
