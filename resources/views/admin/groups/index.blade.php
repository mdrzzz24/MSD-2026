...
<head>
    ...
    <title>Groups — {{ config('app.name') }}</title>
    ...
    <main class="flex-1 lg:ml-64">
        <header class="sticky top-0 z-30 bg-white/80 backdrop-blur border-b border-gray-200">
            <div class="flex items-center justify-between h-16 px-4 sm:px-6 lg:px-8">
                ...
                <a href="{{ route('admin.management.groups.create') }}" class="inline-flex items-center gap-1.5 px-4 py-2 text-xs font-semibold rounded-xl bg-indigo-600 text-white hover:bg-indigo-700 shadow-sm shadow-indigo-200 transition-all">
                        ...
                        <tbody class="divide-y divide-gray-50">
                            @forelse ($groups as $g)
                                <tr class="hover:bg-gray-50/50 transition">
                                    <td class="px-5 py-4">
                                        <span class="text-sm font-semibold text-gray-900">{{ $g->name }}</span>
                                    </td>
                                    <td class="px-5 py-4">
                                    ...
                                    <td class="px-5 py-4">
                                        <div class="flex flex-wrap gap-1">
                                            @php $perms = $g->permissions ?? []; @endphp
                                            @forelse (array_filter($perms) as $key => $val)
                                                <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-xs font-medium bg-indigo-50 text-indigo-600">{{ ucwords(str_replace('_', ' ', $key)) }}</span>
                                            @empty
                                    ...
                                    <td class="px-5 py-4">
                                        <div class="flex items-center justify-end gap-1.5">
                                            <a href="{{ route('admin.management.groups.edit', $g) }}"
                                            ...
                                            <form action="{{ route('admin.management.groups.destroy', $g) }}" method="POST" onsubmit="return confirm('Delete group &quot;{{ $g->name }}&quot;? Users will be unassigned.')">
                                                @csrf @method('DELETE')
                                                <button type="submit"
                                ...
                                <tr>
                                    <td colspan="4" class="px-5 py-16 text-center">
                                        <div class="flex flex-col items-center gap-3">
                                            ...
                                            <a href="{{ route('admin.management.groups.create') }}" class="inline-flex items-center gap-1.5 px-4 py-2 text-xs font-semibold rounded-xl bg-indigo-600 text-white hover:bg-indigo-700 shadow-sm transition">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
                                                Create Group
                                            </a>
                                        </div>
