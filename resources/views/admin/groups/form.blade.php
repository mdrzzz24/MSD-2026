...
<head>
    ...
    <title>{{ isset($group) ? 'Edit' : 'Create' }} Group — {{ config('app.name') }}</title>
    ...
    <main class="flex-1 lg:ml-64">
        <header class="sticky top-0 z-30 bg-white/80 backdrop-blur border-b border-gray-200">
            <div class="flex items-center h-16 px-4 sm:px-6 lg:px-8 gap-4">
                <a href="{{ route('admin.management.groups.index') }}" class="inline-flex items-center gap-1.5 text-sm text-indigo-600 hover:text-indigo-800 font-medium transition">
                ...
                <h1 class="text-lg font-bold text-gray-900">{{ isset($group) ? 'Edit Group' : 'Create Group' }}</h1>
            </div>
        </header>
        <div class="p-4 sm:p-6 lg:p-8 max-w-2xl">
            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                <form action="{{ isset($group) ? route('admin.management.groups.update', $group) : route('admin.management.groups.store') }}" method="POST">
                    @csrf
                    @isset($group) @method('PUT') @endisset

                    <div class="mb-6">
                        <label class="block text-sm font-semibold text-gray-700 mb-1.5">Group Name</label>
                        <input type="text" name="name" value="{{ old('name', $group->name ?? '') }}" required placeholder="e.g. Event Partners"
                            ...
                            @foreach ($permissionList as $key => $label)
                                <label class="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-gray-50 cursor-pointer transition border border-transparent hover:border-gray-100">
                                    <input type="checkbox" name="permissions[{{ $key }}]" value="1"
                                           {{ is_array($currentPerms) && isset($currentPerms[$key]) && $currentPerms[$key] ? 'checked' : '' }}
