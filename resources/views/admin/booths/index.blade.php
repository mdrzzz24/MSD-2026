...
<head>
    ...
    <title>Booths — {{ config('app.name') }}</title>
...
<header class="sticky top-0 z-30 bg-white/80 backdrop-blur border-b border-gray-200">
    <div class="flex items-center justify-between h-16 px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex items-center gap-2">
            <a href="{{ route('admin.booths.export') }}" class="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-medium rounded-xl border border-emerald-200 text-emerald-700 bg-emerald-50 hover:bg-emerald-100 transition" title="Export all booths & their attendees as CSV">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                Export CSV
            </a>
            @if (Auth::user()->canWrite() && Auth::user()->hasPermission('booths'))
            <a href="{{ route('admin.booths.create') }}" class="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-medium bg-indigo-500 text-white rounded-xl hover:bg-indigo-600 shadow-sm shadow-indigo-200 transition">
...
<div class="p-4 sm:p-6 lg:p-8">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            ...
            <p class="text-2xl font-bold text-gray-900 mt-1">{{ $booths->count() }}</p>
        </div>
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            <p class="text-xs text-gray-400 uppercase tracking-wider">Active</p>
            <p class="text-2xl font-bold text-emerald-600 mt-1">{{ $booths->where('is_active', true)->count() }}</p>
        </div>
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            <p class="text-xs text-gray-400 uppercase tracking-wider">Total Visits</p>
            <p class="text-2xl font-bold text-indigo-600 mt-1">{{ $totalVisits }}</p>
        </div>
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            <p class="text-xs text-gray-400 uppercase tracking-wider">Inactive</p>
            <p class="text-2xl font-bold text-gray-400 mt-1">{{ $booths->where('is_active', false)->count() }}</p>
        </div>
    </div>
                ...
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-gray-50">
                        ...
                        <tr class="hover:bg-gray-50/50 transition">
                            ...
                            <td class="px-4 py-3.5"><span class="text-sm font-semibold text-gray-900">{{ $booth->name }}</span></td>
                            <td class="px-4 py-3.5 hidden lg:table-cell"><span class="text-sm text-gray-600 truncate-cell block">{{ $booth->description ?? '—' }}</span></td>
                            ...
                            <td class="px-4 py-3.5 text-center">
                                ...
                                <a href="{{ route('admin.booths.visitors', $booth) }}" class="text-sm font-semibold text-indigo-600 hover:text-indigo-800 hover:underline">
                                    {{ $booth->visits_count ?? 0 }}
                                </a>
                                @else
                                    <span class="text-sm text-gray-600">{{ $booth->visits_count ?? 0 }}</span>
                                @endif
                            </td>
                            <td class="px-4 py-3.5 text-center">
                                @if (Auth::user()->hasPermission('booth_visits'))
                                <a href="{{ route('admin.booths.scan', $booth) }}" class="inline-flex items-center gap-1 px-2.5 py-1.5 text-xs font-medium rounded-lg bg-indigo-50 text-indigo-700 hover:bg-indigo-100 transition">
                            ...
                            <td class="px-4 py-3.5 text-center">
                                <div class="flex items-center justify-center gap-1">
                                    <a href="{{ route('admin.booths.edit', $booth) }}" class="px-2.5 py-1.5 text-xs font-medium rounded-lg bg-amber-50 text-amber-700 hover:bg-amber-100 transition" title="Edit">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                    </a>
                                    <form action="{{ route('admin.booths.toggle', $booth) }}" method="POST" class="inline">
                                        @csrf
                                        <button class="px-2.5 py-1.5 text-xs font-medium rounded-lg {{ $booth->is_active ? 'bg-gray-50 text-gray-600 hover:bg-gray-100' : 'bg-emerald-50 text-emerald-700 hover:bg-emerald-100' }} transition" title="{{ $booth->is_active ? 'Deactivate' : 'Activate' }}">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>
                                        </button>
                                    </form>
                                    <form action="{{ route('admin.booths.destroy', $booth) }}" method="POST" class="inline" onsubmit="return confirm('Delete booth {{ $booth->name }}? This will also delete all visit records.')">
                                        @csrf @method('DELETE')
