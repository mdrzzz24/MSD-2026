...
<head>
    ...
    <title>Scan QR — {{ $booth->name }} — {{ config('app.name') }}</title>
...
<header class="sticky top-0 z-30 bg-white/80 backdrop-blur border-b border-gray-200">
    <div class="flex items-center justify-between h-16 px-4 sm:px-6 lg:px-8">
        <div class="flex items-center gap-4">
            ...
            <h1 class="text-lg font-bold text-gray-900">Scan QR — {{ $booth->name }}</h1>
        </div>
        <a href="{{ route('admin.booths.visitors', $booth) }}" class="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-medium rounded-lg border border-gray-200 text-gray-600 bg-white hover:bg-gray-50 transition">
...
<script>
const scanUrl = '{{ route('admin.booths.scan-process', $booth) }}';
