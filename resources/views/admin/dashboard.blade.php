...
@include('admin.partials.sidebar')
    ...
    <main class="flex-1 lg:ml-64">
        ...
        <div class="p-4 sm:p-6 lg:p-8 space-y-6">
            ...
            {{-- New Agenda Section Toggle (Super Admin only) --}}
            @if (Auth::user()->isSuperAdmin())
            @php $agendaVisible = \Illuminate\Support\Facades\Cache::get('agenda_sections_visible', true); @endphp
            <div class="flex items-center gap-4 bg-white rounded-2xl border border-gray-200 px-5 py-4 shadow-sm">
                <div>
                    <p class="text-sm font-bold text-gray-900">New Agenda Section (Schedule tabs)</p>
                    <p class="text-xs text-gray-500">
                        Status:
                        @if ($agendaVisible)
                            <span class="text-emerald-600 font-semibold">VISIBLE</span> — shown on the home page
                        @else
                            <span class="text-amber-600 font-semibold">HIDDEN</span> — hidden from the home page
                        @endif
                    </p>
                </div>
                <form action="{{ route('admin.toggle-agenda-sections') }}" method="POST" class="ml-auto">
                    @csrf
                    <button type="submit" class="px-4 py-2 text-sm font-semibold rounded-xl transition {{ $agendaVisible ? 'bg-red-500 text-white hover:bg-red-600' : 'bg-emerald-500 text-white hover:bg-emerald-600' }}">
                        {{ $agendaVisible ? 'Hide Agenda Section' : 'Show Agenda Section' }}
                    </button>
                </form>
            </div>
            @endif
            ...
            {{-- ===== ROW 5: Suspected Duplicate Agenda Items (manual cleanup) ===== --}}
            @if (isset($duplicateGroups) && $duplicateGroups->isNotEmpty())
            <div class="bg-white rounded-2xl border border-red-200 shadow-sm overflow-hidden">
                <div class="flex items-center justify-between px-5 py-4 border-b border-red-100 bg-red-50/40">
                    <div>
                        ...
                        <p class="text-xs text-gray-500">
                            {{ $duplicateGroups->count() }} kelompok sesi diduga dobel ({{ $duplicateCount }} salinan berlebih) — hapus yang tidak dipakai
                        </p>
                    </div>
                </div>
                <div class="divide-y divide-gray-100">
                    ...
                    <div class="px-5 py-4">
                        <div class="flex items-center justify-between mb-3">
                            <div class="min-w-0">
                                <p class="text-sm font-bold text-gray-900 truncate">{{ $group->first()->title }}</p>
                                <p class="text-xs text-gray-500 mt-0.5">
                                    {{ $group->count() }} salinan
                                    @if ($group->first()->workshop) &middot; Workshop: {{ $group->first()->workshop->name ?: $group->first()->workshop->title }}@endif
                                    @if ($group->first()->track) &middot; Track: {{ $group->first()->track->name ?: $group->first()->track->title }}@endif
                                </p>
                            </div>
                        </div>
                            ...
                            <table class="w-full text-sm">
                                ...
                                <tbody class="divide-y divide-gray-50">
                                    ...
                                    <tr class="hover:bg-gray-50/50 transition">
                                        <td class="px-4 py-2 font-mono text-xs text-gray-700">#{{ $item->id }}</td>
                                        <td class="px-4 py-2 text-gray-700 whitespace-nowrap">
                                            {{ $item->start_time ? substr($item->start_time, 0, 5) : '—' }} – {{ $item->end_time ? substr($item->end_time, 0, 5) : '—' }}
                                        </td>
                                        <td class="px-4 py-2 text-gray-500">{{ $item->agenda_type ?: ($item->category ?: '—') }}</td>
                                        <td class="px-4 py-2 text-gray-500 whitespace-nowrap">{{ $item->created_at->format('d M Y H:i') }}</td>
                                        <td class="px-4 py-2 text-right">
                                            <form action="{{ route('admin.agenda.destroy', $item) }}" method="POST"
                                                  onsubmit="return confirm('Hapus sesi &quot;{{ $item->title }}&quot; (ID #{{ $item->id }})?')">
                                                @csrf
                                                @method('DELETE')
