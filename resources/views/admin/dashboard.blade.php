            ...
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-4">
                ...
                <div class="lg:col-span-2 bg-gradient-to-br from-indigo-600 to-purple-700 rounded-2xl p-6 text-white shadow-lg">
                    ...
                    <div class="mt-6 flex items-center gap-6">
                        ...
                        <div>
                            <p class="text-3xl font-bold {{ $trend >= 0 ? 'text-emerald-300' : 'text-red-300' }}">
                                {{ $trend >= 0 ? '+' : '' }}{{ $trend }}%
                            </p>
                        ...
                        <div class="h-10 w-px bg-white/20"></div>
                        <div>
                            <p class="text-3xl font-bold text-amber-300">{{ $pending }}</p>
                    ...
                    <div class="space-y-3">
                        <div>
                            <div class="flex justify-between text-xs mb-1">
                                ...
                                <span class="text-gray-500">{{ $approved }} ({{ round($approved/$grand*100) }}%)</span>
                            </div>
                            <div class="h-2 bg-gray-100 rounded-full overflow-hidden">
                                <div class="h-full bg-emerald-500 rounded-full transition-all" style="width: {{ $approved/$grand*100 }}%"></div>
                            </div>
                        </div>
                        <div>
                            <div class="flex justify-between text-xs mb-1">
                                <span class="font-medium text-gray-700">Pending</span>
                                <span class="text-gray-500">{{ $pending }} ({{ round($pending/$grand*100) }}%)</span>
                            </div>
                            <div class="h-2 bg-gray-100 rounded-full overflow-hidden">
                                <div class="h-full bg-amber-400 rounded-full transition-all" style="width: {{ $pending/$grand*100 }}%"></div>
                            </div>
                        </div>
                        <div>
                            <div class="flex justify-between text-xs mb-1">
                                <span class="font-medium text-gray-700">Rejected</span>
                                <span class="text-gray-500">{{ $rejected }} ({{ round($rejected/$grand*100) }}%)</span>
                            </div>
                            <div class="h-2 bg-gray-100 rounded-full overflow-hidden">
                                <div class="h-full bg-red-400 rounded-full transition-all" style="width: {{ $rejected/$grand*100 }}%"></div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-2.5">
                        <a href="{{ route('admin.registrants.index', ['status' => 'pending']) }}"
                           class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-amber-700 bg-amber-50 hover:bg-amber-100 transition">
                            ...
                            @if ($stalePending > 0)
                                <span class="ml-auto text-xs bg-red-100 text-red-600 px-2 py-0.5 rounded-full font-bold">{{ $stalePending }} stale</span>
                            @endif
                        </a>
                        <a href="{{ route('admin.registrants.index') }}"
                        ...
                        @foreach ($chartData as $bar)
                            <div class="flex-1 flex flex-col items-center gap-1 group relative">
                                <div class="w-full flex flex-col-reverse" style="height: 140px;">
                                    ...
                                    @endif
                                    {{-- Pending/total portion minus approved --}}
                       ...
                       class="block bg-white rounded-2xl p-5 border border-gray-100 shadow-sm hover:shadow-md transition group">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-3xl font-bold text-gray-900 mt-1">{{ $workshopCount }}</p>
                            </div>
                        ...
                        <p class="text-xs text-gray-400 mt-2">{{ $workshopRegistrations }} total registrations</p>
                    </a>
                    <a href="{{ route('admin.registrants.index', ['status' => 'pending']) }}"
                       class="block bg-white rounded-2xl p-5 border border-yellow-100 shadow-sm hover:shadow-md transition group">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-3xl font-bold text-yellow-700 mt-1">{{ $pending }}</p>
                            </div>
                        ...
                        @if ($stalePending > 0)
                            <p class="text-xs text-red-500 mt-2 font-medium">⚠ {{ $stalePending }} pending for &gt;2 days</p>
                        @endif
                    </a>
                </div>
            </div>
