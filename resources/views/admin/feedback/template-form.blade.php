...
<head>
    ...
    <title>{{ isset($template) ? 'Edit' : 'Create' }} Template — {{ config('app.name') }}</title>
    ...
    <main class="flex-1 lg:ml-64">
        <header class="sticky top-0 z-30 bg-white/80 backdrop-blur border-b border-gray-200">
            <div class="flex items-center h-16 px-4 sm:px-6 lg:px-8 gap-4">
                <a href="{{ route('admin.feedback.templates') }}" class="inline-flex items-center gap-1.5 text-sm text-indigo-600 hover:text-indigo-800 font-medium transition">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
                    Templates
                </a>
                <span class="text-gray-300">/</span>
                <h1 class="text-lg font-bold text-gray-900">{{ isset($template) ? 'Edit' : 'Create' }} Template</h1>
            </div>
        </header>
        <div class="p-4 sm:p-6 lg:p-8 max-w-3xl">
            @include('admin.partials.notification')

            <form action="{{ isset($template) ? route('admin.feedback.templates.update', $template) : route('admin.feedback.templates.store') }}" method="POST" x-data="templateForm()">
                @csrf
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-5">
                    <div class="mb-4">
                        ...
                        <input type="text" name="name" required
                               ...
                               placeholder="e.g. Workshop Feedback" value="{{ old('name', $template->name ?? '') }}">
                    </div>
                    <div>
                        ...
                        <textarea name="description" rows="2"
                                  ...
                                  placeholder="Optional description">{{ old('description', $template->description ?? '') }}</textarea>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <a href="{{ route('admin.feedback.templates') }}" class="px-4 py-2.5 text-sm font-medium rounded-xl bg-gray-100 text-gray-600 hover:bg-gray-200 transition">Cancel</a>
                </div>
            </form>
        </div>
    </main>
...
<script>
    function templateForm() {
        const existing = @json($existingQuestions);
