        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                <thead><tr class="bg-gray-50/80">
                    ...
                    <th class="px-5 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase hidden md:table-cell">Room</th>
                    ...
                    <th class="px-5 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase hidden lg:table-cell">Registrants</th>
                    ...
                    @forelse ($workshops as $w)
                        <tr class="hover:bg-gray-50/50">
                            <td class="px-5 py-4"><p class="text-sm font-semibold text-gray-900">{{ $w->title }}</p>
                                @if($w->description)<p class="text-xs text-gray-400 mt-0.5 truncate max-w-[200px]">{{ $w->description }}</p>@endif
                            </td>
                            <td class="px-5 py-4 hidden md:table-cell"><span class="text-sm text-gray-600">{{ $w->room ?? '—' }}</span></td>
                            <td class="px-5 py-4"><span class="text-sm text-gray-600 whitespace-nowrap">{{ $w->date->format('d M Y') }}</span></td>
                            <td class="px-5 py-4"><span class="text-sm text-gray-600 whitespace-nowrap">{{ $w->timeRange() }}</span></td>
                            <td class="px-5 py-4 hidden lg:table-cell">
                                <a href="{{ route('admin.workshops.registrants', $w) }}" class="text-sm text-indigo-600 hover:text-indigo-800 font-medium">
                                    {{ $w->registrationsCount() }}/{{ $w->capacity > 0 ? $w->capacity : '∞' }}
                                </a>
                            </td>
                            <td class="px-5 py-4">
