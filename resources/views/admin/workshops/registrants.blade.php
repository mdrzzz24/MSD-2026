                    ...
                    <tbody class="divide-y divide-gray-50">
                        @foreach ($registrants as $r)
                            <tr class="hover:bg-gray-50/50">
                                ...
                                <td class="px-5 py-4">
                                    @if ($r->pivot->utm_source ?? null)
                                        <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold bg-indigo-50 text-indigo-700 border border-indigo-200" title="UTM: {{ $r->pivot->utm_source }} / {{ $r->pivot->utm_medium }} / {{ $r->pivot->utm_campaign }}">
                                            🔗 {{ $r->pivot->utm_source }}
                                        </span>
                                    @else
