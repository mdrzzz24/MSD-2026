{{--
-- Auto-dismiss notification (3 detik) untuk flash messages.
-- Gunakan: @include('admin.partials.notification')
-- Controller: return redirect()->route(...)->with('success', 'Pesan <strong>html</strong>');
-- atau: ->with('error', 'Pesan teks biasa');
--
...
@if (session('success') || session('error'))
    ...
    <div id="notif-{{ $type }}"
        ...
        @endif
        <span class="text-sm flex-1">{!! $isHtml ? $message : e($message) !!}</span>
    ...
    <script>
        (function(){
            var el = document.getElementById('notif-{{ $type }}');
            if(!el) return;
            // Slide in from right
