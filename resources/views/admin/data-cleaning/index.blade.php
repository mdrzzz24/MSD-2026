...
<head>
    ...
    <title>Data Cleaning — {{ config('app.name') }}</title>
...
<p class="mt-1 text-2xl font-bold text-gray-900">{{ number_format($stats['registrants']) }}</p>
</div>
<div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
<p class="text-xs font-semibold text-gray-400 uppercase tracking-wider">With Email</p>
<p class="mt-1 text-2xl font-bold text-indigo-600">{{ number_format($stats['withEmail']) }}</p>
</div>
<div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
<p class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Without Email</p>
<p class="mt-1 text-2xl font-bold text-amber-600">{{ number_format($stats['withoutEmail']) }}</p>
</div>
<div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
<p class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Domain Groups</p>
<p class="mt-1 text-2xl font-bold text-emerald-600">{{ number_format($stats['domains']) }}</p>
</div>
<div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
<p class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Company Variants</p>
<p class="mt-1 text-2xl font-bold text-rose-600">{{ number_format($stats['companyVariants']) }}</p>
</div>
</div>
...
<strong>{{ number_format($stats['domains']) }}</strong> domain group(s) · <strong>{{ number_format($stats['companyVariants']) }}</strong> company name variant(s) to clean up
</p>
</div>

{{-- Domains table --}}
<form id="dataCleaningForm" method="POST" action="{{ route('admin.data-cleaning.apply') }}">
@csrf
@php $oldStandards = old('standard', []); @endphp
...
<tr class="dc-row hover:bg-gray-50/50" data-idx="{{ $loop->index }}" data-domain="{{ $d['domain'] }}">
<td class="px-5 py-3.5 align-top">
<div class="flex items-center gap-2">
<span class="text-sm font-semibold text-gray-900 font-mono">{{ $d['domain'] }}</span>
<span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold bg-indigo-50 text-indigo-700 border border-indigo-200">{{ $d['total'] }} reg</span>
</div>
<button type="button" class="dc-toggle mt-1.5 inline-flex items-center gap-1 text-xs font-medium text-indigo-600 hover:text-indigo-800"
        data-idx="{{ $loop->index }}" data-domain="{{ $d['domain'] }}">
<span class="dc-toggle-caret">▸</span> View members
</button>
</td>
<td class="px-5 py-3.5 align-top">
<div class="flex flex-wrap gap-1.5 max-w-md">
@foreach (array_slice($d['companyCounts'], 0, 8, true) as $name => $count)
@php $isBlank = trim($name) === ''; @endphp
<span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold {{ $isBlank ? 'bg-gray-100 text-gray-500 border border-gray-200' : 'bg-emerald-50 text-emerald-700 border border-emerald-200' }}"
      title="{{ $isBlank ? 'No company value set' : $name }}">
{{ $isBlank ? '(empty)' : \Illuminate\Support\Str::limit($name, 38) }}
<span class="opacity-60">×{{ $count }}</span>
</span>
@endforeach
@if (count($d['companyCounts']) > 8)
<span class="text-[10px] text-gray-400 self-center">+ {{ count($d['companyCounts']) - 8 }} more</span>
@endif
</div>
</td>
<td class="px-5 py-3.5 align-top">
<input type="text" name="standard[{{ $d['domain'] }}]" value="{{ $oldStandards[$d['domain']] ?? $d['suggested'] }}"
...
<tr class="hidden dc-members-row" id="dc-members-{{ $loop->index }}">
<td colspan="4" class="px-5 py-0">
<div class="py-3">
<div class="dc-members-box rounded-xl border border-gray-100 bg-gray-50/60 px-4 py-3" id="dc-members-{{ $loop->index }}-content">
...
<p class="text-xs text-gray-500"><strong>{{ number_format($stats['domains']) }}</strong> domain group(s) · <strong>{{ number_format($stats['registrants']) }}</strong> registrant(s) total</p>
...
try {
const res = await fetch('{{ route('admin.data-cleaning.members') }}', {
method: 'POST',
headers: {
