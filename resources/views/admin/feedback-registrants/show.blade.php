...
<head>
    ...
    <title>Feedback — {{ $registrant->display_name ?: $registrant->name }} — {{ config('app.name') }}</title>
    ...
    <main class="flex-1 lg:ml-64">
        <header class="sticky top-0 z-30 bg-white/80 backdrop-blur border-b border-gray-200">
            <div class="flex items-center h-16 px-4 sm:px-6 lg:px-8 gap-3">
                ...
                <h1 class="text-lg font-bold text-gray-900 truncate">{{ $registrant->display_name ?: $registrant->name }}</h1>
            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                <div class="flex items-start gap-4">
                    <div class="w-14 h-14 rounded-full bg-gradient-to-br from-indigo-400 to-purple-500 flex items-center justify-center text-white text-xl font-bold flex-shrink-0">
                        {{ strtoupper(mb_substr($registrant->display_name ?: $registrant->name, 0, 1)) }}
                    </div>
                    <div class="min-w-0 flex-1">
                        <div class="flex items-center gap-2 flex-wrap">
                            <h2 class="text-xl font-bold text-gray-900">{{ $registrant->display_name ?: $registrant->name }}</h2>
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold capitalize {{ $registrant->status === 'approved' ? 'bg-emerald-50 text-emerald-700' : ($registrant->status === 'pending' ? 'bg-amber-50 text-amber-700' : 'bg-red-50 text-red-700') }}">
                                {{ $registrant->status }}
                            </span>
                        </div>
                        ...
                        @if ($registrant->job_title || $registrant->job_role)
                            <p class="text-xs text-gray-400 mt-1.5">
                                {{ collect([$registrant->job_title, $registrant->job_role])->filter()->implode(' · ') }}
                            </p>
                        @endif
                    </div>
                    <div class="text-right flex-shrink-0">
                        <p class="text-3xl font-bold text-indigo-600">{{ $feedbacks->count() }}</p>
                        <p class="text-xs text-gray-400">session{{ $feedbacks->count() === 1 ? '' : 's' }} with feedback</p>
                    </div>
                </div>
            </div>
                        ...
                        <div class="px-6 py-4 border-b border-gray-100 flex items-start justify-between gap-3 flex-wrap">
                            <div class="min-w-0">
                                <div class="flex items-center gap-2 flex-wrap">
                                    ...
                                    @if ($agendum?->room)
                                        <span class="text-xs text-gray-400">{{ $agendum->room }}</span>
                                    @endif
                                    @if ($agendum)
                                        <span class="text-xs text-gray-400">{{ $agendum->timeLabel() }}</span>
                                    @endif
                                </div>
                                <h3 class="text-sm font-bold text-gray-900 mt-1">{{ $displayTitle }}</h3>
                                <p class="text-[11px] text-gray-400 mt-0.5">Submitted {{ $fb->created_at ? $fb->created_at->format('d M Y, H:i') : '—' }}</p>
                            </div>
                            @if ($agendum)
                            <a href="{{ route('feedback.form', $agendum) }}" target="_blank" rel="noopener"
