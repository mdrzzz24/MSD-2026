    ...
    <main class="flex-1 lg:ml-64">
        ...
        <div class="p-4 sm:p-6 lg:p-8">
            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 mb-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        @if ($activeTemplate)
                            <p class="text-xs text-gray-400 mt-0.5">"{{ $activeTemplate->name }}" — {{ $activeTemplate->subject }}</p>
                        @else
            ...
            <form method="POST" action="{{ route('admin.email-logs.send-reminder') }}" id="reminderForm">
                ...
                {{-- Registrants grouped by registration date --}}
                @forelse ($dateGroups as $date => $group)
                    @php
                        $remindedCount = $group->filter(fn ($r) => in_array($r->id, $remindedIds ?? [], true))->count();
                        $notRemindedCount = $group->count() - $remindedCount;
                    @endphp
                    <details class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mb-4" {{ $loop->first ? 'open' : '' }}>
                        <summary class="px-5 py-3.5 flex items-center justify-between cursor-pointer hover:bg-gray-50 transition select-none list-none">
                            <div class="flex items-center gap-3 flex-wrap">
                                <span class="inline-flex items-center gap-1.5 text-sm font-bold text-gray-900">
                                    ...
                                    {{ $date === 'Unknown' ? 'Unknown date' : \Carbon\Carbon::parse($date)->format('l, d M Y') }}
                                </span>
                                <span class="text-xs text-gray-400 grp-count">{{ $group->count() }} registrant{{ $group->count() === 1 ? '' : 's' }}</span>
                                <span class="text-xs text-gray-300">·</span>
                                <span class="inline-flex items-center gap-1 text-xs font-semibold text-emerald-600"><span class="w-1.5 h-1.5 rounded-full bg-emerald-500"></span><span class="grp-reminded">{{ $remindedCount }}</span> reminded</span>
                                <span class="text-xs text-gray-300">·</span>
                                <span class="inline-flex items-center gap-1 text-xs font-semibold text-gray-500"><span class="w-1.5 h-1.5 rounded-full bg-gray-400"></span><span class="grp-not">{{ $notRemindedCount }}</span> not reminded</span>
                            </div>
                                ...
                                <tbody class="divide-y divide-gray-50">
                                    @foreach ($group as $r)
                                        ...
                                        <tr class="hover:bg-gray-50/50 transition {{ $alreadyReminded ? 'bg-emerald-50/50' : '' }}"
                                            data-search="{{ strtolower($r->name) }}|{{ strtolower($r->email) }}|{{ strtolower($r->unique_code ?? '') }}"
                                            data-reminded="{{ $alreadyReminded ? '1' : '0' }}">
                                            <td class="px-5 py-3">
