...
<head>
    ...
    <title>Add General Session — {{ config('app.name') }}</title>
    ...
    <form action="{{ route('admin.general-sessions.store') }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <textarea name="key_highlights" rows="3"
                      class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500"
                      placeholder="One bullet point per line…">{{ old('key_highlights') }}</textarea>
        </div>
        <div class="flex items-center gap-4 pt-2">
            <label class="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" name="is_full_row" value="1" {{ old('is_full_row') ? 'checked' : '' }} class="w-4 h-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
