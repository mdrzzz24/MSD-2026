...
<body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
    <div style="background: #f0fdf4; border: 1px solid #bbf7d0; border-radius: 8px; padding: 30px;">
        ...
        <p>Halo <strong>{{ $registrant->display_name }}</strong>,</p>
        ...
        <div style="background: #fff; border: 1px solid #ddd; border-radius: 8px; padding: 16px; margin: 16px 0;">
            ...
            <p style="margin: 4px 0;"><strong>Password:</strong> <code style="background: #f3f4f6; padding: 2px 8px; border-radius: 4px;">{{ $plainPassword }}</code></p>
        </div>
        <p>Anda dapat login di: <a href="{{ route('registrant.login') }}">{{ route('registrant.login') }}</a></p>
