...
<head>
    ...
    <title>Edit Track Session — <?php echo e(config('app.name')); ?></title>
...
<header class="sticky top-0 z-30 bg-white/80 backdrop-blur border-b border-gray-200">
    <div class="flex items-center h-16 px-4 sm:px-6 lg:px-8 gap-4">
        <a href="<?php echo e(route('admin.track-sessions.index')); ?>" class="inline-flex items-center gap-1.5 text-sm text-indigo-600 hover:text-indigo-800 font-medium transition">
...
<div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 sm:p-8">
    ...
    <?php endif; ?>
    <form action="<?php echo e(route('admin.track-sessions.update', $agendum)); ?>" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <select name="track_id" required class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500">
                ...
                <?php $__currentLoopData = $tracks; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $tr): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <option value="<?php echo e($tr->id); ?>" <?php echo e(old('track_id', $agendum->track_id) == $tr->id ? 'selected' : ''); ?>><?php echo e($tr->name ?: $tr->title); ?></option>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </select>
        </div>
        <div>
            <label class="block text-sm font-semibold text-gray-700 mb-1">Title <span class="text-red-500">*</span></label>
            <input type="text" name="title" value="<?php echo e(old('title', $agendum->title)); ?>" required
        ...
        <div>
            ...
            <textarea name="description" rows="4"
                      class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500"
                      placeholder="Session description…"><?php echo e(old('description', $agendum->description)); ?></textarea>
        </div>
        ...
        <div>
            ...
            <textarea name="key_highlights" rows="3"
                      class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500"
                      placeholder="One bullet point per line…"><?php echo e(old('key_highlights', $agendum->key_highlights ?? '')); ?></textarea>
        </div>
        ...
        <div class="flex items-center gap-3 pt-2">
            ...
            <a href="<?php echo e(route('admin.track-sessions.index')); ?>" class="text-sm text-gray-500 hover:text-gray-700">Cancel</a>
        </div>
    </form>
    <div class="mt-4 pt-4 border-t border-gray-100">
        <form action="<?php echo e(route('admin.track-sessions.destroy', $agendum)); ?>" method="POST" onsubmit="return confirm('Delete this session?');" class="inline">
...
<?php /**PATH /Users/mdrz/2026/MSD26/resources/views/admin/track-sessions/edit.blade.php ENDPATH**/ ?>