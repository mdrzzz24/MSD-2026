...
<header class="sticky top-0 z-30 bg-white/80 backdrop-blur border-b border-gray-200">
    <div class="flex items-center justify-between h-16 px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex items-center gap-2">
            <a href="<?php echo e(route('admin.workshops.registrants.export-csv', array_merge(['workshop' => $workshop], request()->only(['profile', 'source', 'date_from', 'date_to'])))); ?>"
...
<div class="p-4 sm:p-6 lg:p-8">
    ...
    <form method="GET" action="<?php echo e(route('admin.workshops.registrants', $workshop)); ?>" class="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 mb-6">
        <div class="flex flex-wrap items-end gap-3">
            ...
            <div class="flex items-center gap-2">
                ...
                <?php if(request('profile') || request('source') || request('date_from') || request('date_to')): ?>
                    <a href="<?php echo e(route('admin.workshops.registrants', array_merge(['workshop' => $workshop], request()->except(['profile', 'source', 'date_from', 'date_to'])))); ?>" class="px-4 py-2 text-xs font-medium rounded-xl bg-gray-100 text-gray-600 hover:bg-gray-200 transition">Clear</a>
                <?php endif; ?>
            </div>
        </div>
    </form>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-100 flex flex-wrap items-center justify-between gap-2">
            ...
            <div class="flex items-center gap-3">
                ...
                <span class="text-xs text-gray-400">Canc: <strong class="text-gray-600"><?php echo e($registrants->where('pivot.status', 'cancelled')->count()); ?></strong></span>
            </div>
        </div>
                ...
                <table class="w-full table-fixed">
                    ...
                    <tbody class="divide-y divide-gray-50">
                        <?php $__currentLoopData = $registrants; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $r): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            ...
                            <tr class="hover:bg-gray-50/50 transition <?php echo e($wsStatus === 'cancelled' ? 'bg-gray-100/70' : ''); ?>">
                                ...
                                <td class="px-4 py-3.5 text-center">
                                    <?php if($showTrackSelect && !($r->cancelled ?? false)): ?>
                                ...
                                <td class="px-4 py-3.5 text-center">
                                    ...
                                    <?php if($r->hasClientRemark()): ?>
                                        <div class="mt-1.5 flex flex-col items-center gap-1">
                                            <span class="inline-flex items-center px-1.5 py-0.5 rounded-md text-[10px] font-semibold border <?php echo e($r->client_remark_action === 'approve' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : ($r->client_remark_action === 'reject' ? 'bg-red-50 text-red-700 border-red-200' : 'bg-orange-50 text-orange-700 border-orange-200')); ?>">
                                            ...
                                            <?php if($r->clientRemarkedBy): ?>
                                                <span class="text-[10px] font-medium text-gray-600"><?php echo e($r->clientRemarkedBy->name); ?></span>
                                            <?php endif; ?>
                                ...
                                <td class="px-4 py-3.5 text-center">
                                    <?php if($r->pivot->created_at): ?>
                                        <?php $joinedAt = \Carbon\Carbon::parse($r->pivot->created_at)->addHours(7); ?>
                                        <span class="text-xs text-gray-600" title="<?php echo e($joinedAt->format('d M Y H:i')); ?>"><?php echo e($joinedAt->format('d M, H:i')); ?></span>
