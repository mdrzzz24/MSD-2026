...
<?php
    $selectedSources = array_values(array_filter((array) request('source', [])));
    $selectedCount = count($selectedSources);
?>
<div>
    <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Source</label>
    <div class="relative filter-dropdown" data-empty-label="All sources">
        <button type="button" data-dropdown-toggle
            ...
            <span class="text-gray-600 truncate" data-dropdown-label><?php echo e($selectedCount ? $selectedCount . ' selected' : 'All sources'); ?></span>
        ...
        <div data-dropdown-panel class="absolute z-30 mt-1.5 w-64 bg-white border border-gray-200 rounded-xl shadow-lg hidden">
            <div class="flex items-center justify-between px-3 py-2 border-b border-gray-100">
                <span class="text-[10px] font-semibold text-gray-400 uppercase tracking-wide">Select sources</span>
            ...
            <div class="max-h-52 overflow-y-auto p-1.5">
                <label class="flex items-center gap-2.5 px-2 py-1.5 rounded-lg hover:bg-gray-50 cursor-pointer select-none text-sm text-gray-700">
                    <input type="checkbox" name="source[]" value="direct"
                           <?php if(in_array('direct', $selectedSources, true)): echo 'checked'; endif; ?>
                           class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                    <span class="truncate">Direct (no UTM source)</span>
                </label>
                <?php $__currentLoopData = $sources; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $s): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <label class="flex items-center gap-2.5 px-2 py-1.5 rounded-lg hover:bg-gray-50 cursor-pointer select-none text-sm text-gray-700">
                        <input type="checkbox" name="source[]" value="<?php echo e($s); ?>"
                               <?php if(in_array($s, $selectedSources, true)): echo 'checked'; endif; ?>
                               class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                        <span class="truncate"><?php echo e($s); ?></span>
                    </label>
...
<?php /**PATH /Users/mdrz/2026/MSD26/resources/views/admin/partials/source-filter.blade.php ENDPATH**/ ?>