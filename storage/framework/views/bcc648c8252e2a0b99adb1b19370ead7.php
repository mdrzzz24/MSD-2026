...
<script>
    ...
    function liveSearch(input) {
        ...
        searchTimer = setTimeout(async () => {
            ...
            try {
                ...
                if (pag) pag.innerHTML = data.pagination ? data.pagination + '<p class="text-xs text-gray-500 mt-2">Showing (' + (data.total || 0) + ') sessions</p>' : '';
    ...
    // Works for both the initial render and AJAX-re-rendered rows (the row
    // onclick calls this global function directly).
    ...
    function renderQrResult(data) {
        ...
        if (data.sessions.length === 0) {
            ...
            data.sessions.forEach(s => {
                ...
                html += '</div>';
                // Answers — visible to admins only; viewers just see who filled it
                if (qrCanWrite && s.answers.length > 0) {
