...
<head>
    ...
    <title>Scan QR — <?php echo e($booth->name); ?> — <?php echo e(config('app.name')); ?></title>
...
<header class="sticky top-0 z-30 bg-white/80 backdrop-blur border-b border-gray-200">
    <div class="flex items-center justify-between h-16 px-4 sm:px-6 lg:px-8">
        <div class="flex items-center gap-4">
            ...
            <h1 class="text-lg font-bold text-gray-900">Scan QR — <?php echo e($booth->name); ?></h1>
        </div>
        <a href="<?php echo e(route('admin.booths.visitors', $booth)); ?>" class="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-medium rounded-lg border border-gray-200 text-gray-600 bg-white hover:bg-gray-50 transition">
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            ...
            <div id="panelManual" class="space-y-4">
                <div>
                    ...
                    <p class="text-xs text-gray-400 mt-2">Type or paste the QR token from the registrant's badge, then press Enter or click Scan.</p>
                </div>
                <div class="bg-indigo-50 border border-indigo-100 rounded-xl p-4">
                    <p class="text-sm font-medium text-indigo-800 mb-2">Need a registrant's QR code?</p>
                    <a href="<?php echo e(route('admin.registrants.index')); ?>" target="_blank" class="text-xs text-indigo-600 hover:text-indigo-800 underline">Browse registrants →</a>
                    <span class="text-xs text-gray-400 mx-2">or</span>
                    <a href="<?php echo e(route('admin.management.qr')); ?>" target="_blank" class="text-xs text-indigo-600 hover:text-indigo-800 underline">View all QR codes →</a>
                </div>
            </div>
...
<script>
const scanUrl = '<?php echo e(route('admin.booths.scan-process', $booth)); ?>';
...
function startCamera() {
    ...
    if (typeof Html5Qrcode === 'undefined') {
        showResult('error', 'QR scanner library not loaded. Please refresh the page.');
        return;
    }
    ...
    html5QrCode.start(
        ...
        cameraRunning = false;
        showResult('error', 'Camera access denied or not available. Please use Manual input instead.');
        switchTab('manual');
    });
}
...
function onScanSuccess(decodedText, decodedResult) {
    ...
    processScan();
    // Switch back to manual tab so camera stops
    switchTab('manual');
...
function processScan() {
    ...
    if (!token) {
        showResult('error', 'Please enter a QR token.');
        input.focus();
        return;
    }

    // If token looks like a URL, extract the last segment
    ...
    .then(data => {
        if (data.success) {
            if (data.already_visited) {
                showResult('warning', data.message, data.registrant);
            } else {
                showResult('success', data.message, data.registrant);
            }
            input.value = '';
        } else {
...
function showResult(type, message, registrant) {
    ...
    if (type === 'loading') {
        ...
        html = '<div class="text-center py-6">' +
            ...
            '<p class="text-sm font-semibold text-emerald-700 mb-1">Visit Recorded!</p>' +
        ...
        html = '<div class="text-center py-6">' +
            ...
            '<p class="text-sm font-semibold text-amber-700 mb-1">Already Visited</p>' +
            '<p class="text-sm text-gray-600 mb-4">' + message + '</p>';
        if (registrant) {
            html += '<div class="bg-gray-50 rounded-xl p-4 text-left text-sm"><span class="text-gray-400">Visited at:</span> <span class="font-semibold text-gray-700">' + (registrant.visited_at || '-') + '</span></div>';
        }
        html += '</div>';
    } else {
        html = '<div class="text-center py-6">' +
...
<?php /**PATH /Users/mdrz/2026/MSD26/resources/views/admin/booths/scan.blade.php ENDPATH**/ ?>