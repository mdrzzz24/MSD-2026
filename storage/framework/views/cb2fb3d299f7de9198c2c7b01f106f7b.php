...
<body class="bg-[#060b26] font-sans antialiased min-h-screen">
    ...
    <div class="w-full max-w-2xl mx-auto px-4 sm:px-6 py-8 sm:py-12" x-data="{ showLogin: <?php echo json_encode($needsLogin, 15, 512) ?>, authMode: 'password' }">
            ...
            <div class="flex items-center justify-center gap-2 mt-3 flex-wrap">
                <?php if($agendum->start_time): ?>
                    <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg bg-white/5 border border-white/10 text-xs text-indigo-200">
                        ...
                        <?php echo e($agendum->timeLabelWith($timeSlots)); ?>
        ...
        <div class="bg-white/[0.06] backdrop-blur-xl rounded-3xl border border-white/10 shadow-2xl overflow-hidden" x-data="feedbackForm()">
            ...
            <div class="flex items-center gap-3 px-6 sm:px-8 py-4 border-b border-white/10 bg-white/[0.03]">
                <div class="w-9 h-9 rounded-full bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center text-white text-sm font-bold flex-shrink-0">
                    <?php echo e(mb_strtoupper(mb_substr($registrant->display_name, 0, 1))); ?>

                </div>
                <div class="min-w-0">
                    <p class="text-sm font-semibold text-white truncate"><?php echo e($registrant->display_name); ?></p>
                    <p class="text-xs text-indigo-300 truncate"><?php echo e($registrant->email); ?></p>
                </div>
                ...
                <div class="px-6 sm:px-8 py-6 sm:py-8">
                    <div class="mb-6 p-4 rounded-xl bg-emerald-500/15 border border-emerald-400/30 flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-emerald-200/70 mt-0.5">Submitted on <?php echo e($existingFeedback->created_at->format('d M Y, H:i')); ?></p>
                        </div>
                    </div>

                    <?php if($existingFeedback->answers->count() > 0): ?>
                                ...
                                <div class="p-5 rounded-2xl bg-white/[0.04] border border-white/10">
                                    <div class="flex items-start gap-3">
                                        <div class="flex-1 min-w-0">
                                            <p class="text-sm font-semibold text-white"><?php echo e($q->question_text); ?></p>
                                            <div class="mt-2">
                                                <?php if($q->question_type === 'rating'): ?>
                                                    <div class="flex items-center gap-0.5">
                                                        <?php for($i = 1; $i <= ($q->rating_max ?: 5); $i++): ?>
                                                            <svg class="w-4 h-4 <?php echo e($i <= (int) $answer->answer_value ? 'text-amber-400' : 'text-white/15'); ?>" fill="currentColor" viewBox="0 0 20 20">
                                                                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                                                            </svg>
                                                        <?php endfor; ?>
                                                        <span class="ml-2 text-xs text-amber-300/90 font-semibold"><?php echo e($answer->answer_value); ?>/<?php echo e($q->rating_max ?: 5); ?></span>
                                                    </div>
                                                <?php elseif($q->question_type === 'yes_no'): ?>
                                                    <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full <?php echo e($answer->answer_value === 'yes' ? 'bg-emerald-500/15 text-emerald-300 border border-emerald-400/30' : 'bg-rose-500/15 text-rose-300 border border-rose-400/30'); ?> text-xs font-semibold">
                                                        <?php echo e(ucfirst($answer->answer_value)); ?>

                                                    </span>
                                                <?php else: ?>
                                                    <p class="text-sm text-indigo-200 leading-relaxed"><?php echo e($display); ?></p>
                                                <?php endif; ?>
                                            </div>
                                        </div>
                                    </div>
            ...
            <?php else: ?>
            <form action="<?php echo e(route('feedback.store', $agendum)); ?>" method="POST" class="px-6 sm:px-8 py-6 sm:py-8">
                <?php echo csrf_field(); ?>
                ...
                <?php if($questions->count() > 0): ?>
                    ...
                    <?php $__currentLoopData = $questions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $q): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        ...
                        <div class="mb-5 p-5 rounded-2xl border <?php echo e($q->parent_question_id ? 'ml-3 sm:ml-5 border-l-2 border-l-indigo-400/50 bg-indigo-500/[0.06] border-white/10' : 'bg-white/[0.04] border-white/10'); ?>" x-show="isVisible(<?php echo e($q->id); ?>)" x-cloak>
                            <div class="mb-4">
                                <label class="block text-sm font-semibold text-white leading-snug">
                                    <?php echo e($q->question_text); ?>

                                    <?php if($q->required): ?> <span class="text-rose-400">*</span> <?php endif; ?>
                            ...
                            <?php elseif($q->question_type === 'rating'): ?>
                                <?php $ratingMax = $q->rating_max ?: 5; $starSize = $ratingMax > 5 ? 'w-5 h-5' : 'w-7 h-7'; ?>
                                <div class="rounded-2xl bg-white/[0.03] border border-white/10 p-4">
                                    <div class="flex flex-wrap items-center justify-center gap-1.5">
                                        <?php for($i = 1; $i <= $ratingMax; $i++): ?>
                                            <button type="button" @click="setRating(<?php echo e($q->id); ?>, <?php echo e($i); ?>)"
                                                    ...
                                                    :title="'<?php echo e($i); ?> / <?php echo e($ratingMax); ?>'">
                                                <svg class="<?php echo e($starSize); ?>" :class="getRating(<?php echo e($q->id); ?>) >= <?php echo e($i); ?> ? 'text-amber-400 drop-shadow' : 'text-white/15 hover:text-white/30'" fill="currentColor" viewBox="0 0 20 20">
                                                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                                                </svg>
                                            </button>
                                        <?php endfor; ?>
                                    ...
                                    <div class="flex items-center justify-between mt-2.5 text-[11px] text-indigo-300/60">
                                        <span>1</span>
                                        <span class="font-semibold text-amber-300/90" x-text="(getRating(<?php echo e($q->id); ?>) || 0) + ' / <?php echo e($ratingMax); ?>'"></span>
                                        <span><?php echo e($ratingMax); ?></span>
                                    </div>
                                ...
                                <div class="space-y-2.5">
                                    <?php $__currentLoopData = $q->options ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $opt): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <label class="flex items-center gap-3 px-4 py-3 rounded-xl border cursor-pointer transition"
                                               :class="isPicked(<?php echo e($q->id); ?>, $el.querySelector('input').value) ? 'border-indigo-400/60 bg-indigo-500/15' : 'border-white/10 bg-white/[0.04] hover:bg-white/[0.08]'">
                                    ...
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                    <?php if($q->allow_other): ?>
                                        <label class="flex items-center gap-3 px-4 py-3 rounded-xl border cursor-pointer transition"
                                               :class="isPicked(<?php echo e($q->id); ?>, $el.querySelector('input').value) ? 'border-indigo-400/60 bg-indigo-500/15' : 'border-white/10 bg-white/[0.04] hover:bg-white/[0.08]'">
                                            <input type="radio" :name="'answers[<?php echo e($q->id); ?>]'" value="__other__"
                                                   x-model="answers[<?php echo e($q->id); ?>]"
                                                   :required="isVisible(<?php echo e($q->id); ?>) && <?php echo e($isReq); ?>"
                                                   <?php echo e(old('answers.' . $q->id) === '__other__' ? 'checked' : ''); ?>

                                                   class="accent-indigo-500 w-4 h-4 flex-shrink-0">
                                            <span class="text-sm text-white">Other</span>
                                        </label>
                                        <div x-show="isOtherVisible(<?php echo e($q->id); ?>)" x-cloak class="pl-2 pt-1">
                                            <input type="text" name="other_answers[<?php echo e($q->id); ?>]"
                                                   x-model="otherTexts[<?php echo e($q->id); ?>]"
                                                   value="<?php echo e(old('other_answers.' . $q->id)); ?>"
                            ...
                            <?php elseif($q->question_type === 'multi_choice'): ?>
                                <div class="space-y-2.5">
                                    ...
                                    <?php $__currentLoopData = $q->options ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $opt): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <label class="flex items-center gap-3 px-4 py-3 rounded-xl border cursor-pointer transition"
                                               :class="isPicked(<?php echo e($q->id); ?>, $el.querySelector('input').value) ? 'border-indigo-400/60 bg-indigo-500/15' : 'border-white/10 bg-white/[0.04] hover:bg-white/[0.08]'">
                                            <input type="checkbox" name="answers[<?php echo e($q->id); ?>][]" value="<?php echo e($opt); ?>"
                                                   @change="toggleMulti(<?php echo e($q->id); ?>, $el.value)"
                                                   :checked="isSelected(<?php echo e($q->id); ?>, $el.value)"
                                                   <?php echo e(in_array($opt, (array) old('answers.' . $q->id, [])) ? 'checked' : ''); ?>
                                    ...
                                    <?php if($q->allow_other): ?>
                                        <label class="flex items-center gap-3 px-4 py-3 rounded-xl border cursor-pointer transition"
                                               :class="isPicked(<?php echo e($q->id); ?>, $el.querySelector('input').value) ? 'border-indigo-400/60 bg-indigo-500/15' : 'border-white/10 bg-white/[0.04] hover:bg-white/[0.08]'">
                                            <input type="checkbox" name="answers[<?php echo e($q->id); ?>][]" value="__other__"
                                                   @change="toggleMulti(<?php echo e($q->id); ?>, $el.value)"
                                                   :checked="isSelected(<?php echo e($q->id); ?>, $el.value)"
                                                   <?php echo e(in_array('__other__', (array) old('answers.' . $q->id, [])) ? 'checked' : ''); ?>
                            ...
                            <?php elseif($q->question_type === 'yes_no'): ?>
                                <div class="grid grid-cols-2 gap-3">
                                    <label class="flex items-center justify-center gap-2 px-4 py-3 rounded-xl border cursor-pointer transition"
                                           :class="isPicked(<?php echo e($q->id); ?>, $el.querySelector('input').value) ? 'border-emerald-400/60 bg-emerald-500/15' : 'border-white/10 bg-white/[0.04] hover:bg-white/[0.08]'">
                                    ...
                                    <label class="flex items-center justify-center gap-2 px-4 py-3 rounded-xl border cursor-pointer transition"
                                           :class="isPicked(<?php echo e($q->id); ?>, $el.querySelector('input').value) ? 'border-rose-400/60 bg-rose-500/15' : 'border-white/10 bg-white/[0.04] hover:bg-white/[0.08]'">
...
unset($__errorArgs, $__bag); ?>
                        ...
                        <div class="flex items-center gap-2" x-data="{ rating: <?php echo e(old('rating', 0)); ?> }">
                            <?php for($i = 1; $i <= 5; $i++): ?>
                                <button type="button" @click="rating = <?php echo e($i); ?>"
                                    ...
                                    <svg class="w-8 h-8" :class="rating >= <?php echo e($i); ?> ? 'text-amber-400' : 'text-white/20'" fill="currentColor" viewBox="0 0 20 20">
                                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                                    </svg>
                                </button>
                            <?php endfor; ?>
                ...
                <div class="relative w-full max-w-md bg-white rounded-2xl shadow-2xl p-8 animate-modal-in">
                    ...
                    <div x-show="authMode === 'password'">
                        <form action="<?php echo e(route('login.attempt')); ?>" method="POST" class="space-y-5">
                            <?php echo csrf_field(); ?>
                            <input type="hidden" name="redirect" value="<?php echo e(request()->getRequestUri()); ?>">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="email" id="fb_email" name="email" value="<?php echo e(old('email')); ?>" required autofocus
    ...
    <script>
        function feedbackForm() {
            ...
            const multiKeys = <?php echo json_encode($questions->where('question_type', 'multi_choice')->pluck('id')->toArray() ?? [], 512) ?>;
            ...
            return {
                answers: {},
                otherTexts: {},
                questionCount: <?php echo json_encode($questions->count(), 15, 512) ?>,
                answeredCount() {
                    let c = 0;
