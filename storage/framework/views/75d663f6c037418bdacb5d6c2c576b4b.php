                ...
                <select id="existingSourceSelect" class="w-full px-4 py-2.5 bg-white border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500" onchange="onExistingSourceSelect(this)">
                    ...
                    <optgroup label="🗂️ Tracks">
                        ...
                        <option value="track_<?php echo e($tr->id); ?>"
                            ...
                            data-session-title="<?php echo e(e($fs?->title ?? '')); ?>"
                            data-session-desc="<?php echo e(e($fs?->description ?? '')); ?>"
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="agenda_type" id="agendaTypeSelect" class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 focus:bg-white transition">
                        <option value="">— None —</option>
                        <option value="track" <?php echo e(old('agenda_type', request('agenda_type'))==='track'?'selected':''); ?>>Track</option>
                ...
                <div>
                    ...
                    <?php $trackList = \App\Models\Track::with(['agendaItems' => fn($q) => $q->orderBy('start_time')->orderBy('order')])->orderBy('title')->get(); ?>
        ...
        <div class="border-t border-gray-100 pt-4 mt-2">
            ...
            <div class="mb-4">
                ...
                <div id="speakerOrderList" class="space-y-1.5 border border-gray-200 rounded-xl p-2 bg-gray-50 min-h-[40px]">
                    <?php $__empty_2 = true; $__currentLoopData = $orderedSpeakerModels; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $osp): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_2 = false; ?>
                ...
                <div class="space-y-3 max-h-80 overflow-y-auto border border-gray-200 rounded-xl p-3 bg-gray-50">
                    ...
                    <div class="flex items-start gap-3 p-2 rounded-lg hover:bg-white transition">
                        <label class="flex items-center gap-2 cursor-pointer flex-shrink-0 pt-1">
                            <input type="checkbox" value="<?php echo e($sp->id); ?>" data-name="<?php echo e($sp->name); ?>" <?php echo e(in_array($sp->id, old('speaker_ids', [])) ? 'checked' : ''); ?> onchange="toggleSpeaker(this)" class="w-4 h-4 rounded border-gray-300 text-indigo-600 speaker-check">
                        </label>
