                    ...
                    <div class="p-6 space-y-4">
                        ...
                        <div class="bg-indigo-50 rounded-xl p-4 border border-indigo-200">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <button <?php echo e($registrant->hasClientRemark() ? '' : "onclick=openRejectReason($registrant->id,this)"); ?>

                                        id="detail-reject-<?php echo e($registrant->id); ?>"
                                        data-name="<?php echo e($registrant->name); ?>"
                                        class="flex-1 px-4 py-3 rounded-xl text-sm font-bold transition border-2
                            ...
                            <div class="mt-3 text-xs text-gray-500 text-center">
                                <span class="font-semibold <?php echo e($registrant->clientRemarkTextClass()); ?>">
                                    <?php echo e($registrant->clientRemarkLabel()); ?>

                                </span>
                                <?php if($registrant->client_remark): ?>
                                    &middot; <?php echo e($registrant->client_remark); ?>
                        ...
                        <div class="bg-indigo-50 rounded-xl p-4 border border-indigo-200">
                            ...
                            <div class="flex flex-col items-start gap-1">
                                <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold <?php echo e($registrant->clientRemarkBadgeClass()); ?>">
                                    <?php echo e($registrant->clientRemarkLabel(true)); ?>
                                ...
                                <?php endif; ?>
                                <?php if($registrant->client_remark): ?>
                                    <span class="text-xs text-gray-700"><?php echo e($registrant->client_remark); ?></span>
                                <?php endif; ?>
                        ...
                        <form action="<?php echo e(route('admin.registrants.toggle-waitlist', $registrant)); ?>" method="POST" class="mt-3">
                            <?php echo csrf_field(); ?>
                            <button type="submit"
                                    onclick="return confirm('<?php echo e($registrant->isWaitlisted() ? 'Remove from waiting list?' : 'Add to waiting list?'); ?>')"
                                    title="<?php echo e($registrant->isWaitlisted() ? 'Remove from waiting list' : 'Mark as waiting list'); ?>"
                                    class="w-full inline-flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-sm font-bold transition border-2
                                    <?php echo e($registrant->isWaitlisted() ? 'bg-orange-500 text-white border-orange-500 hover:bg-orange-600' : 'bg-white text-orange-700 border-orange-300 hover:bg-orange-50'); ?>">
                                ...
                                <?php echo e($registrant->isWaitlisted() ? 'Remove Waiting List' : 'Waiting List'); ?>
                        ...
                        <div class="p-5 space-y-4">
                            ...
                            <div class="bg-indigo-50 rounded-xl p-3 border border-indigo-200">
                                ...
                                <div class="flex items-center gap-1.5 mb-1">
                                    <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold <?php echo e($registrant->clientRemarkBadgeClass()); ?>">
                                        <?php echo e($registrant->clientRemarkLabel()); ?>
...
<script>
    ...
    // ═══════════════════════════════
    const clientRemarkCard = document.getElementById('clientRemarkCard');
    const clientRemarkDisplay = document.getElementById('clientRemarkDisplay');
    const clientRemarkForm = document.getElementById('clientRemarkForm');
    const editClientRemarkBtn = document.getElementById('editClientRemarkBtn');
    const clientRemarkInput = document.getElementById('clientRemarkInput');
    const clientRemarkStatus = document.getElementById('clientRemarkStatus');

    function toggleClientRemarkEdit() {
        if (!clientRemarkDisplay || !clientRemarkForm) return;
        clientRemarkDisplay.classList.add('hidden');
        clientRemarkForm.classList.remove('hidden');
        if (editClientRemarkBtn) editClientRemarkBtn.classList.add('hidden');
        if (clientRemarkInput) {
            clientRemarkInput.focus();
            clientRemarkInput.setSelectionRange(clientRemarkInput.value.length, clientRemarkInput.value.length);
        }
    }

    function cancelClientRemarkEdit() {
        if (!clientRemarkDisplay || !clientRemarkForm) return;
        clientRemarkDisplay.classList.remove('hidden');
    ...
    async function saveClientRemark() {
        const remark = clientRemarkInput.value.trim();
        const actionEl = document.querySelector('input[name="client_remark_action"]:checked');
        const action = actionEl ? actionEl.value : 'approve';
        ...
        try {
            ...
            if (res.ok && data.success) {
                // Rebuild display
                var actionLabel = action === 'approve' ? '✅ Recommend Approve' : '❌ Recommend Reject';
                var actionClass = action === 'approve' ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700';
                clientRemarkDisplay.innerHTML = '<div class="flex items-center gap-2 mb-1.5">'
                    + '<span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold ' + actionClass + '">' + actionLabel + '</span>'
                    + '<span class="text-xs text-gray-400">' + data.updatedAt + '</span>'
                    + '</div>'
                    + '<dd class="text-sm text-gray-800 whitespace-pre-wrap">' + escapeHtml(remark) + '</dd>';

                clientRemarkDisplay.classList.remove('hidden');
                clientRemarkForm.classList.add('hidden');
                if (editClientRemarkBtn) {
                    editClientRemarkBtn.classList.remove('hidden');
                    editClientRemarkBtn.textContent = '✏️ Update';
                }

                // Flash effect
                if (clientRemarkCard) {
                    clientRemarkCard.classList.add('ring-2', 'ring-indigo-400');
                    setTimeout(function() {
                        if (clientRemarkCard) clientRemarkCard.classList.remove('ring-2', 'ring-indigo-400');
                    }, 1500);
                }
            } else {
                clientRemarkStatus.textContent = data.error || 'Failed to save.';
            }
        } catch (err) {
            clientRemarkStatus.textContent = 'Failed to save. ' + (err.message || err);
        } finally {
            btn.disabled = false;
            btn.textContent = origText;
        }
    ...
    // ═══════════════════════════════
    var detailMustBeUrl = '<?php echo e(url('admin/registrants/' . $registrant->id . '/client-remark')); ?>';
    ...
    <div class="fixed inset-0 flex items-center justify-center p-4">
        ...
        <div class="p-6">
            ...
            <select id="ppSelect" class="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-amber-100 focus:border-amber-300 outline-none bg-white">
                <option value="">— Select a reason —</option>
                <?php $__currentLoopData = config('client_reasons.pending'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $reason): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <option value="<?php echo e($reason); ?>"><?php echo e($reason); ?></option>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </select>
            <div class="flex justify-end gap-2.5 mt-5">
